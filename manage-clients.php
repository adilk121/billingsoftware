...
$res=db_query($sql);	
if($res>0){
  db_query("update tbl_invoice_order set invoice_order_status='Inactive' where invoice_order_client_id='$catID' ");
...
$res=db_query($sql);	
if($res>0){
db_query("update tbl_invoice_order set invoice_order_status='Active' where invoice_order_client_id='$catID' ");
...
if(is_post_back()) {
   $count="";
	...
	if(is_array($arr_ids)) {
      ...
      db_query("update tbl_invoice_order set invoice_order_status='Active' where invoice_order_client_id in ($str_ids)");
       ...
       db_query("update tbl_invoice_order set invoice_order_status='Inactive' where invoice_order_client_id in ($str_ids)");
...
if($_REQUEST['search_value']!="")
{
	...
	$search_con="and (client_cid='$value' OR client_company_name like '%$value%' OR client_email like '%$value%' OR client_mobile like '%$value%' OR client_name like '%$value%') ";
}
...
{
  $filter_con="and client_status='Active'";
}

if($_REQUEST['list_filter']=="Inactive")
{
  $filter_con="and client_status='Inactive'";
}
...
$sql = "select * from  tbl_clients where 1 $search_con $filter_con";
...
<?}*/?>     
            ...
            <section class="content">
               <div class="row">
                   ...
                   <input type="text" placeholder="CID/Name/Company Name/Email/Mobile" value="<?=$_REQUEST['search_value']?>" name="search_value" class="form-control text-center" style="width:300px; float: left;" required />
                   ...
                   <a href="manage-clients.php">Clear</a>
                   <?}?>
               </form>
</p>
...
<script type="text/javascript">
 ...
 {
  window.location.href="manage-clients.php?list_filter="+filter_value;
 }
...
<strong>Address: </strong><?=$client_billing_address?>
 ...
 <td class="v-middle" align="center">
  ...
  <a href="manage-orders-invoices.php?order_client_id=<?=$line_raw['client_id']?>" ><span style="color:white !important;" class="label-primary label label-default">
...
<br>
<a href="add-edit-order-invoice.php?id=0&client_ID=<?=$line_raw['client_id']?>" ><span style="color:black !important;" class="label-warning label label-default"><i class="fa fa-plus"></i> Add Order/Invoice</span></a>
...
<a href="manage-clients.php?st=1&pid=<?=$line_raw["client_id"]?>"><span class="label-custom label label-default">Active</span></a>
<?php }else{?>
<a href="manage-clients.php?st=0&pid=<?=$line_raw["client_id"]?>"><span class="label-danger label label-default">Inactive</span></a>
<?php }?>
...
<td class="text-center v-middle">
<a href="add-edit-client.php?id=<?=$line_raw["client_id"]?>"><button type="button" class="btn btn-add btn-sm" ><i class="fa fa-pencil"></i></button>
</a>                                          
</td>
